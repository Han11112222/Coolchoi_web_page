...
with col3:
    # 마인드스포츠 상장 및 트로피 사진 (루트 경로 파일명 직접 입력)
    try:
        st.image("KakaoTalk_20260518_195539069_03.jpg", caption="마인드스포츠 동상 및 국가대표 인증서", use_container_width=True)
    except:
        st.write("사진 로딩 중...")
...
with col3:
    # 이화령 정상 도착 사진과 인증서 사진을 탭으로 나누어 보여주기
    tab1, tab2 = st.tabs(["이화령 정상 정복", "종주 인증서"])
    with tab1:
        try:
            st.image("KakaoTalk_20260518_195539069_07.jpg", caption="백두대간 이화령 정상에서", use_container_width=True)
        except:
            pass
    with tab2:
        try:
